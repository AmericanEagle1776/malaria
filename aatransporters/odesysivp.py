...
def f(t, y, told):  # tlist, xdictionary):  # timestep function
    ...
    dp1dt = + npo.polyval(t - (p1_read_time / 360), p1_expr_profile) * mean_expr * 360 * p1_init_rate - r1
    dpedt = - pdecay_rate * np.exp(-pdecay_rate * t) * pe0
    dp1depdt = p1i * pdecay_rate * np.exp(-delta_t * pdecay_rate)
    dp2dt = + npo.polyval(t - (p2_read_time / 360), p2_expr_profile) * mean_expr * 360 * p2_init_rate - r2
